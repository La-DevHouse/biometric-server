...
- **Migración de huellas entre dispositivos: RESUELTA (2026-09-07), verificada físicamente contra dos equipos.** Lo decisivo es **de dónde se lee** el template: hay que usar `GET_USER_INFO` (blob limpio de **612 bytes**), nunca `GET_ENROLL_DATA` (blob de 524 bytes, contaminado con memoria sin inicializar del equipo origen — con ese el dedo no matchea nunca). Con la forma limpia, patchar el `user_id` embebido en el offset 608 y escribir con `SET_ENROLL_DATA` (agrega una huella a un usuario existente, sin tocar su ficha) o `SET_USER_INFO` (crea el usuario completo; sobre uno existente dispara el reindexado destructivo). Ambas escrituras verificadas físicamente. Ver `docs/05-commands-catalog.md` → "Migración de huellas entre dispositivos".
