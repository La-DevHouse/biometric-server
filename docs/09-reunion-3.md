...
| 7   | Timestamp de asociación device↔empresa/sede                                                                                                                                              | **P2** | ✅ Hecho (2026-09-21): `assignDeviceAction` estampa `company_linked_at` al cambiar empresa/sede, se limpia al desasignar; visible en el detalle del device como "Asignado desde". §3.13 |
| 8   | Auditar todas las server actions: que **ninguna** borre en duro empresa/empleado/contrato; degradar a `status=inactive`                                                                  | **P0** | D6                                                                                          |
| 9   | Confirmar/crear enum de estado de contrato con `retirado` además de `inactive`, o documentar que `inactive` cubre "retirado"                                                             | **P2** | ✅ Decidido (2026-09-21): **no hace falta un tercer estado.** `employment_status` se queda en `{active, inactive}`. "Retirado" = `inactive` **con** `end_date` seteado (lo que ya hace `endEmploymentAction` en cada baja) — Ezequiel nunca pidió distinguir "inactivo temporal" de "retirado permanente", solo que el historial no se borre, y `end_date != null` ya es esa marca. Un contrato nuevo tras un re-ingreso es una fila `employment` nueva, no una reactivación de la vieja. [0179] |
...
| 14  | Definir estándar del campo "ID del equipo" del device y documentarlo en `05`/`06`                                                                       | **P2** | ✅ Hecho (2026-09-21): documentado en `03-device-network.md` §4 — el sistema identifica por número de serie (`dev_id`), el campo es cosmético; estándar RIF+sufijo (`-2`, `-3`) si hay varios equipos por sede. Q8 |
...
| #   | Acción                                                                                       | Prio   | Notas                                                 |
| --- | -------------------------------------------------------------------------------------------- | ------ | ----------------------------------------------------- |
| 24  | Parseo de **RIF (PDF)** → autocompletar empresa (RIF, razón social, dirección)               | **P1** | ✅ Hecho (2026-09-21): `lib/rifParser.ts` (regex determinístico) + `lib/pdfText.ts` (`pdf-parse`) + botón en `CompanyFormDialog`. Validado contra un RIF real y con tests unitarios. Ojo: `pdf-parse`/pdf.js necesita `serverExternalPackages` en `next.config.ts` + `import "pdf-parse/worker"` antes de importarlo, si no falla "Setting up fake worker failed" al correr empaquetado en el server de Next |
| 25  | OCR/IA de **cédula (foto)** → autocompletar empleado (nombre, fecha nac., cédula, dirección) | **P1** | ✅ Hecho (2026-09-21): Gemini Flash (tier gratis) vía `lib/vision/gemini.ts` + `lib/documentVision.ts` (capa swappable de proveedor) + `DocumentCameraCapture` (guía tipo blueprint) en `EmployeeFormDialog`. La foto se conserva en `employee.cedula_photo` (decisión explícita del cliente). Mismo mecanismo agregado para **RIF por foto** en `CompanyFormDialog` (alternativa al PDF), vía `extractRifPhotoAction`. Validado con una llamada real a Gemini contra la cédula real de Jesús (extracción 100% correcta) y con un RIF sintético renderizado a imagen. Script de prueba: `npm run test-vision -- <cedula\|rif> <ruta-imagen>` |
